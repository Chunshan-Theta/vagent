...
/**
 * 相關的 env 變數
 * NEWDEAN_ACCOUNT_LIST_FILE : 指定用戶名單的檔案路徑，格式 => [{account: "帳號", password: "密碼"}, {account: "帳號", password: "密碼"}]
 * 
 */
...
export async function POST(request: Request) {
  try {
    ...
    const v = url.searchParams.get('v') || 'v1'; // 預設使用 v1
...
async function loadAccountList(v:string) : Promise<DemoAccount[]> {
  ...
  let filePath = '';
  if( process.env.NEWDEAN_ACCOUNT_LIST_FILE && v === 'v1' ){
    filePath = process.env.NEWDEAN_ACCOUNT_LIST_FILE;
  }
  if (!filePath) {
    return [
